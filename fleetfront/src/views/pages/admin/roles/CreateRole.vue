...
onMounted(async () => {
    ...
    const res = await roleStore.getPermissionsNameList();
...
const onSubmit = async () => {
    ...
    const res = await roleStore.createRoles(formData.value, toast);
    if (res) {
        router.push({ name: 'roles' });
    }
    isSubmitting.value = false; // re-enable after request
};
