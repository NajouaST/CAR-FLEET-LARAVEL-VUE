...
// dialogs
const departementDialog = ref(false);
const deleteDepartementDialog = ref(false);
...
// fetch departements
const fetchDepartements = async (params = {}) => {
    loading.value = true;
    try {
        const data = await paramsGenereauxStore.getDepartements(params, toast);
...
function openNew() {
    ...
    departementDialog.value = true;
}

function hideDialog() {
    departementDialog.value = false;
    submitted.value = false;
}

async function saveDepartement() {
    submitted.value = true;
    if (!departement.value.nom?.trim()) return;
    ...
    try {
        ...
        if (departement.value.id) {
            ...
            await paramsGenereauxStore.createDepartement(formData, toast);
            toast.add({ severity: "success", summary: "Created", detail: "departement created", life: 3000 });
        }

        departementDialog.value = false;
        fetchDepartements(lazyParams.value);
    } catch (err) {
        console.error('Failed to save departement:', err);
        if (err.response?.status === 422) {
            ...
            toast.add({ severity: "error", summary: "Error", detail: "Failed to save departement", life: 3000 });
        }
    }
}

function editDepartement(s) {
    departement.value = { ...s };
    departementDialog.value = true;
}

function confirmDeleteDepartement(s) {
    departement.value = s;
    deleteDepartementDialog.value = true;
}

async function deleteDepartement() {
    await paramsGenereauxStore.deleteDepartement(departement.value.id, toast);
    deleteDepartementDialog.value = false;
    toast.add({ severity: "success", summary: "Deleted", detail: "departement deleted", life: 3000 });
    fetchDepartements(lazyParams.value);
}
</script>
    ...
    <div class="grid grid-cols-12 gap-8">
        <div class="col-span-12">
            <div class="card flex justify-between items-center">
                <h1 class="text-muted-color font-medium">departements : {{ totalRecords }} </h1>
        ...
        <div class="col-span-12">
            <div class="card">
                ...
                >
                    <template #empty>No departements found.</template>
                ...
                <Dialog v-model:visible="departementDialog" :style="{ width: '50vw', maxWidth: '600px' }" header="departement Details" modal>
                    <div class="space-y-4">
                        <div>
                            <label for="nom" class="block font-medium mb-2">Nom *</label>
                            <InputText id="nom" v-model.trim="departement.nom" required :invalid="submitted && !departement.nom" class="w-full" />
                            <small v-if="submitted && !departement.nom" class="text-red-500">Nom is required.</small>
                        </div>
                    </div>

                    <template #footer>
                        <Button label="Cancel" icon="pi pi-times" text @click="hideDialog" />
                        <Button label="Save" icon="pi pi-check" @click="saveDepartement" />
                    </template>
                </Dialog>

                <!-- Delete Dialog -->
                <Dialog v-model:visible="deleteDepartementDialog" header="Confirm" modal>
                    <div class="flex items-center gap-4">
                        <i class="pi pi-exclamation-triangle !text-3xl" />
                        <span v-if="departement">Are you sure you want to delete <b>{{ departement.nom }}</b>?</span>
                    </div>
                    <template #footer>
                        <Button label="No" icon="pi pi-times" text @click="deleteDepartementDialog = false" />
                        <Button label="Yes" icon="pi pi-check" @click="deleteDepartement" />
                    </template>
                </Dialog>
            </div>
        </div>
