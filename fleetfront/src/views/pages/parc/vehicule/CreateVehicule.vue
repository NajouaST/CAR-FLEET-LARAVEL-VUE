...
// 🔥 Watch marque selection -> fetch modèles
watch(
    () => vehicule.value.marque_id,
    async (newMarqueId) => {
        if (newMarqueId) {
            modeles.value = (
                await paramsParcStore.getModeles(
    ...
    () => vehicule.value.formule_achat,
    (newVal) => {
        if (newVal === "fond_propre") {
            vehicule.value.loyer = null; // reset
        }
    }
);

const selectedModele = computed(() => {
    return modeles.value.find((m) => m.id === vehicule.value.modele_id) || null;
});
        ...
        <form @submit.prevent="saveVehicule" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <Image v-if="carteGriseFrontPreview" class="mt-2">
                        <img :src="carteGriseFrontPreview" alt="Carte grise recto" class="w-40 rounded shadow" preview  />
                    </Image>
                </div>
                <div>
